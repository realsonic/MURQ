# Заметки к выпускам MURQ.Console 0.9.x
...
| **Конец локации** | `end`
| **Присвоение значения переменной** | `a=4` <hr/> `b="привет"` <hr/> `c=a` | В правой части поддерживаются арифметические выражения.
| **Условие**   | `if a=1 then pln Ну, здравствуй! else pln А теперь прощай!`   | _Пока поддерживается только проверка равенства значений арифметических выражений._
