...
## 0.8.0
### Добавлено
- Поддержка подстановок в любом месте кода.
- Поддержка объединений команд с помощью `&`.
- Поддержка переносов строк с помощью `_`. На следующей строке `_` должна отделяться от кода пробелом или табуляцией.
- Время загрузки квеста теперь указывается после названия файла: `Файл: dungeon.qst, загружен за 00:00.103`.
### Изменено
