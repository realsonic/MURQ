...
## 0.8.0
### Добавлено
- Поддержка подстановок в любом месте кода, кроме меток.
...
- Поддержка `else` в `if`, но пока `else` не является терминатором для `p`|`pln` (`pln` съедает всё до конца строки или `&`).
