...
## 0.9.0
### Добавлено
- Арифметика: сложение `+`, вычитание `-`, умножение `*`, деление `/`, скобки `(` и `)` и унарные операции `+` и `-`.

## 0.8.1
### Исправлено
- Действия операторов `goto` и `end` выполняются мгновенно, и все продолжения (после `&` или на следующей строке) игнорируются. 
- Фантомные кнопки (указывающие на несуществующие метки) помечаются как ` // phantom` и не нажимаются.
- Исправлена ошибка: если внутри многострочного комментария встречался `*`, то комментарий закрывался при встрече `/` на любом расстоянии от `*`.

## 0.8.0
### Добавлено
- Поддержка подстановок в любом месте кода, кроме строк-меток.
- В подстановках можно писать такое же выражение, как в коде. Сейчас это имя переменной, число или строка в кавычках.
- Поддержка объединений команд с помощью `&`. Она является терминатором для команд, заканчивающихся открытым текстом (`p`/`pln`, `btn`, `goto`).
- Поддержка `else` в `if`.
- Поддержка переносов строк с помощью `_`. На следующей строке `_` должна отделяться от последующего кода пробелом или табуляцией.
- Время загрузки квеста теперь указывается после названия файла в формате `минуты:секунды.миллисекунды`: `Файл: dungeon.qst, загружен за 00:00.103`.
### Изменено
- Распознавание (и анализ на ошибки) URQL выполняется теперь при выполнении строки квеста, а не при загрузке всего квеста, как раньше. Единственная ошибка, которая может возникнуть при загрузке квеста - пустые метки.
### Исправлено
- При выполнении `end` забывается текущая строка квеста (т.е. следующая строка уже не будет выполнена).
- При попытке перехода по фантомной метке (по кнопке или `goto`) не меняется строка для выполнения. (Кнопки и так уже нажимаются в режиме ожидания (после `end` нет строки для выполнения), а для `goto` с фантомной ссылкой это приведёт к тому, что перехода и не будет и выполнение продолжится тут же.)
### Улучшено
- Загрузка квеста и выполнение инструкций значительно ускорены.
...
## 0.5.0
Простые подстановки в `p`/`pln`, `goto`, `rnd`_число_, `perkill`, загрузка файла квеста в разных кодировках.
### Добавлено
...
- Поддержка `goto` (без параметров):
  ...
  goto метка
  ```
- Поддержка системной переменной `rnd`_число_:
  ```urql
